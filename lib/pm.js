...
function spawn(script, options = {}) {
    ...
    const joinedEnv = Object.assign({}, process.env, { FORCE_COLOR: supportsColor });
    const cwd = options.cwd;

    return new Promise((resolve, reject) => {
        // On Linux, the child must be spawned detached because we have to kill the whole process group later in kill()
        const detached = process.platform === 'linux';
        
        const childProcess = crossSpawn(command, realArgs, { env: joinedEnv, cwd, detached });
...
function kill(child, signal = 'SIGTERM') {
    return new Promise((resolve, reject) => {
        ...
        if (process.platform === 'win32') {
            ...
            // handles killing of recursive children
            spawn(`kill -${signal} -- -${child.pid}`).catch(() => {});
        } else {
            child.kill(signal);
        }
    });
