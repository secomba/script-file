...
function execScript(script, args) {
    ...
    case 'string': {
        return pm.spawn(script, { args });
    }
    case 'object': {
        if (Array.isArray(script)) {
            return sequential(script.map(childScript => () => execScript(childScript, args)));
        }
        if (script.isParallel) {
            return Promise.all(script.script.map(childScript => execScript(childScript, args)));
        }
...
function exec(scriptName, args) {
    return Promise.resolve(resolve(scriptName))
        .then(script => execScript(script, args))
        .catch((error) => {
